...
public class seamCalculate {
	...
	private void generalCalculation() {
		...
		updateCoors(coors);
		/*
		 * while (row_index >= 0) { x = coors[coors_index - 1].col; y =
		 * coors[coors_index - 1].row; currCell = energyObj.cellMatrix[x + 1][y + 1];
		 * upLeftCell = energyObj.cellMatrix[x][y]; upCell = energyObj.cellMatrix[x +
		 * 1][y]; upRightCell = energyObj.cellMatrix[x + 2][y];
		 * System.out.println("current=" + currCell + ", x=" + x + ", y=" + y);
		 * coors[coors_index].row = row_index; if (x == 0) {// first col if
		 * (min(upCell.M, upRightCell.M) == upCell.M) { coors[coors_index].col = 0; }
		 * else { coors[coors_index].col = 1; } } else if (x == cols - 1) {// last col
		 * if (min(upCell.M, upLeftCell.M) == upCell.M) { coors[coors_index].col = cols
		 * - 1; } else { coors[coors_index].col = cols - 2; } } else { if (min(upCell.M,
		 * min(upLeftCell.M, upRightCell.M)) == upCell.M) { coors[coors_index].col = x;
		 * } else if (min(upRightCell.M, upLeftCell.M) == upRightCell.M) {
		 * coors[coors_index].col = x + 1; } else { coors[coors_index].col = x - 1; } }
		 * coors_index++; row_index--; }
		 */
	...
	public Coordinates[][] pick_seams(int k) {
		...
		//	System.out.println("i=" + i+ ", rows="+energyObj.rows);
